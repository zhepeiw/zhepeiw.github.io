...
date: '2019-05-24'
updatedDate: 2019-05-24T00:00:00.000Z
tags:
  - Sound Recognition
  - Neural Networks
---
In this article we provide a brief overview of our work [Multi-View Networks For Multi-Channel Audio Classification](https://arxiv.org/pdf/1811.01251.pdf). In this work we propose a neural network architecture that performs audio classification/event detection given recordings from multiple devices.
...
# Multi-View Networks
Multi-View Networks(MVNs) are variants of Recurrent Neural Networks(RNNs), which are a common starting point for classifying ore predicting sequences(if you're not familiar with RNNs, you may refer to [this awesome post](http://colah.github.io/posts/2015-08-Understanding-LSTMs/)). Recall that an RNN may take sequences of arbitrary length since it unrolls across the time dimension. In other words, an RNN can be trained on sequences of length $$k$$ and be tested on sequences of length $$k'$$ where $$k \neq k'$$.
...
If you're interested in the experimental setups and results, please refer to [the paper](https://arxiv.org/pdf/1811.01251.pdf) for a more detailed description. You may also check out [our poster](/assets/images/blogs/blog2/multi-view-icassp.pdf), which was presented at ICASSP 2019. The source code for MVN will be released within a few months.
